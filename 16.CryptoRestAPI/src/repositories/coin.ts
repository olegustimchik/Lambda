...
import { CoinTypeInsert, CoinTypeSelect } from "../types/tables.ts";
...
export class CoinRepository {
    ...
    insertOne = async (coin: CoinTypeInsert): Promise<void | MySqlRawQueryResult> => {
        return this.dbConnection
            .insert(coins)
            .values({ coinSymbol: coin.coinSymbol })
            .onDuplicateKeyUpdate({ set: { coinSymbol: coin.coinSymbol } });
    };

    insertMany = async (coinList: CoinTypeInsert[]): Promise<void | MySqlRawQueryResult> => {
        return this.dbConnection
            .insert(coins)
            .values(coinList)
            .onDuplicateKeyUpdate({ set: { coinSymbol: sql`coin_symbol` } });
    };
